...
<x-page-header :title="'Receipt batch #'.str_pad((string) $intakeBatch->id, 6, '0', STR_PAD_LEFT)" description="Each receipt is processed independently and remains private to you.">
    <x-slot:actions><a class="btn btn-primary" href="{{ route('expense-receipt-intakes.create') }}">Upload another batch</a></x-slot:actions>
</x-page-header>
...
<div class="card mb-4"><div class="card-body d-flex flex-wrap gap-4">
    ...
    <div><span class="detail-label">Receipts</span><strong>{{ $intakeBatch->documentIntakes->count() }}</strong></div>
...
<div class="intake-progress-grid mb-4">
    @foreach ([App\DocumentIntakeStatus::Pending, App\DocumentIntakeStatus::Processing, App\DocumentIntakeStatus::NeedsVerification, App\DocumentIntakeStatus::Failed] as $status)
