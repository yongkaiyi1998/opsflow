...
@forelse ($spendCategories as $spendCategory)<tr><td>{{ $spendCategory->name }}</td><td><code>{{ $spendCategory->code }}</code></td><td><x-status-badge :status="$spendCategory->status" /></td><td class="text-end"><a class="btn btn-sm btn-outline-primary" href="{{ route('spend-categories.edit', $spendCategory) }}">Edit</a></td></tr>@empty<tr><td class="text-center text-secondary py-5" colspan="4">No spend categories found.</td></tr>@endforelse
