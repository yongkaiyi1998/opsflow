...
@forelse ($vendors as $vendor)<tr><td>{{ $vendor->name }}</td><td>{{ $vendor->code ?? '—' }}</td><td><div>{{ $vendor->email ?? '—' }}</div>@if ($vendor->phone)<small class="text-secondary">{{ $vendor->phone }}</small>@endif</td><td><x-status-badge :status="$vendor->status" /></td><td class="text-end"><a class="btn btn-sm btn-outline-primary" href="{{ route('vendors.edit', $vendor) }}">Edit</a></td></tr>@empty<tr><td class="text-center text-secondary py-5" colspan="5">No vendors found.</td></tr>@endforelse
