@php
    ...
    $items = old('items', $savedItems ?: ($formDefaults['items'] ?? [['description' => '', 'quantity' => '1.0000', 'unit_price' => '']]));
    $fieldValue = fn (string $field) => old($field, $supplierInvoice?->{$field} ?? ($formDefaults[$field] ?? null));
@endphp
...
<div class="row g-3">
    <div class="col-md-6"><label class="form-label" for="vendor_id">Vendor</label><select class="form-select @error('vendor_id') is-invalid @enderror" id="vendor_id" name="vendor_id" required><option value="">Select vendor</option>@foreach ($vendors as $vendor)<option value="{{ $vendor->id }}" @selected((string) $fieldValue('vendor_id') === (string) $vendor->id)>{{ $vendor->name }}</option>@endforeach</select></div>
    <div class="col-md-6"><label class="form-label" for="invoice_no">Supplier invoice number</label><input class="form-control @error('invoice_no') is-invalid @enderror" id="invoice_no" name="invoice_no" value="{{ $fieldValue('invoice_no') }}" maxlength="100" required></div>
    <div class="col-md-6"><label class="form-label" for="department_id">Department</label><select class="form-select @error('department_id') is-invalid @enderror" id="department_id" name="department_id" required><option value="">Select department</option>@foreach ($departments as $department)<option value="{{ $department->id }}" @selected((string) $fieldValue('department_id') === (string) $department->id)>{{ $department->name }}</option>@endforeach</select></div>
    <div class="col-md-6"><label class="form-label" for="category_id">Spend category</label><select class="form-select @error('category_id') is-invalid @enderror" id="category_id" name="category_id" required><option value="">Select category</option>@foreach ($categories as $category)<option value="{{ $category->id }}" @selected((string) $fieldValue('category_id') === (string) $category->id)>{{ $category->name }}</option>@endforeach</select>@if (config('ai.enabled') && isset($categorySuggestionUrl))<button class="btn btn-sm btn-link px-0 mt-1" type="submit" formaction="{{ $categorySuggestionUrl }}" formnovalidate>Suggest category</button>@endif</div>
    <div class="col-md-6"><label class="form-label" for="invoice_date">Invoice date</label><input class="form-control @error('invoice_date') is-invalid @enderror" id="invoice_date" name="invoice_date" type="date" value="{{ old('invoice_date', $supplierInvoice?->invoice_date?->toDateString() ?? ($formDefaults['invoice_date'] ?? '')) }}" required></div>
    <div class="col-md-6"><label class="form-label" for="due_date">Due date</label><input class="form-control @error('due_date') is-invalid @enderror" id="due_date" name="due_date" type="date" value="{{ old('due_date', $supplierInvoice?->due_date?->toDateString() ?? ($formDefaults['due_date'] ?? '')) }}"></div>
    <div class="col-12"><label class="form-label" for="description">Description</label><textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4" maxlength="10000" required>{{ $fieldValue('description') }}</textarea></div>
</div>
@if (isset($categorySuggestionUrl))
