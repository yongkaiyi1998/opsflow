...
- OpsFlow-specific rules in this file, TASKS.md, ARCHITECTURE.md, and docs/ take precedence over generic Laravel Boost guidance when they conflict.
- Laravel Boost guidance should be used for Laravel/framework conventions and version-specific APIs.
- Do not expand task scope merely because Boost recommends additional optional files or features.
- Follow the current TASKS.md task boundary and acceptance criteria.
- Create factories when they are useful for tests; do not automatically create seeders unless the current task requires them.
- Do not ask whether to create optional Laravel artifacts when the task scope already makes the requirement clear.
